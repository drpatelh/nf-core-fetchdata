# nf-core/fetchdata: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/fetchdata/usage](https://nf-co.re/fetchdata/usage)
...
```console
nextflow run nf-core/fetchdata --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/fetchdata
```
...
First, go to the [nf-core/fetchdata releases page](https://github.com/nf-core/fetchdata/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
